...
if (isset($_POST["submit"])) {
    ...
    $sql = "INSERT INTO jadwal_kuliah (id_kelas, id_matkul, id_dosen, id_hari, id_jam_kuliah, id_ruangan, semester, tahun_ajaran) VALUES ('$id_kelas', '$id_matkul', '$id_dosen', '$id_hari', '$id_jam_kuliah', '$id_ruangan', '$semester', '$tahun_ajaran')";
...
function buildOptions($table, $idField, $namaField) {
    global $conn;
    $options = "";
    $query = "SELECT $idField, $namaField FROM $table";
    $res = $conn->query($query);
    while ($r = $res->fetch_assoc()) {
        $options .= "<option value='".$r[$idField]."'>".$r[$namaField]."</option>";
    }
    return $options;
}
...
<div class="content" style="margin-top: 70px;">
    <form action="" method="post" class="border border-success p-4 rounded shadow">
        ...
        <div class="mb-3">
            ...
            <?= buildOptions("matkul","id", "namaMatkul");?>
            </select>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-4 text-end">
                ...
                <button type="submit" class="btn btn-secondary mb-3" onclick="window.history.back()">Kembali</button>
            </div>
        </div>
    </form>
</div>
